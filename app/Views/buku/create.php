<div class="page-heading"><div><h1>Tambah Buku</h1><p>Kelola informasi buku perpustakaan.</p></div></div>
<div class="card" style="max-width:760px;">
  <div class="card-header bg-white fw-semibold">Tambah Buku</div>
  <div class="card-body">
    <form action="<?= BASE_URL ?>/buku/store" method="POST" novalidate>
      <div class="mb-3">
        ...
        <input type="text" name="kode_buku" class="form-control <?= $this->errors('kode_buku') ? 'is-invalid' : '' ?>"
               value="<?= htmlspecialchars($this->old('kode_buku')) ?>" placeholder="Contoh: BK006">
      ...
      <div class="row">
        <div class="col-md-6 mb-3">
          ...
          <input type="number" name="tahun_terbit" class="form-control <?= $this->errors('tahun_terbit') ? 'is-invalid' : '' ?>"
                 value="<?= htmlspecialchars($this->old('tahun_terbit')) ?>">
        ...
        <div class="col-md-6 mb-3">
          ...
          <input type="number" name="stok" min="0" class="form-control <?= $this->errors('stok') ? 'is-invalid' : '' ?>"
                 value="<?= htmlspecialchars($this->old('stok')) ?>">
          <?php if ($this->errors('stok')): ?><div class="invalid-feedback"><?= htmlspecialchars($this->errors('stok')) ?></div><?php endif; ?>
        </div>
      </div>
      <div class="form-actions"><a href="<?= BASE_URL ?>/buku" class="btn btn-secondary">Batal</a><button type="submit" class="btn btn-primary">Simpan Buku</button></div>
    </form>
  </div>
</div>
